...
check "installed: commit-msg stub"  'grep -q "never-again" .git/hooks/commit-msg'
check "installed: resolver entry"   'grep -q "_after.sh" .claude/settings.json'
check "installed: codex entries"    'grep -q "never-again/launch" .codex/hooks.json'
...
check "commit-msg stub gone"        '[ ! -f .git/hooks/commit-msg ]'
